...
io.on("connection", (socket: Socket) => {
	...
	socket.on("crear-partida", async ({ nombre, numeroLetras, email }) => {
		...
		const idPartida = listaDePartidas.addPartida(
			nombre,
			socket.id,
			email,
		...
		if (idPartida !== 0) {
			...
			io.sockets.emit(
				...
				jugadoresConectados.getJugadores()
			);
		}
	});
	...
	socket.on("unirse-a-partida", ({ email, idPartida }) => {
		listaDePartidas
			.getPartida(Number(idPartida))
			.addJugador({ email, idSesion: socket.id, partida: idPartida });
		jugadoresConectados.addJugador(email, socket.id, idPartida);
		...
		socket.emit(
			"comienza-juego",
			listaDePartidas.getPartida(Number(idPartida))
		);
		socket.broadcast
...
const palabraAleatoria = async (): Promise<string> => {
	try {
		...
		const palabraTabla = await Palabra.findByPk(

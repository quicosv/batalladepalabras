...
export const PalabraPage = () => {
  ...
  const { form, onInputChange, onResetForm } = useForm<ITuPalabra>({
    ...
    tuPalabra: '',
    partidas_idPartida: Number(idPartida)
  });
  ...
  socket?.on('comienza-juego', (partida: IPartida) => {
    ...
    setContrincante(partida.jugadores[0].email);
  });
  ...
  useEffect(() => {
    ...
    socket?.emit('unirse-a-partida', { email, idPartida });
  }, [idPartida]);
      ...
      {empiezaPartida ? (
        <form className="row g-3" onSubmit={procesaPalabra}>
          <div className="form-group">
            <label className="form-label" htmlFor="palabra">
        ...
        <p>{contrincante === '' ? 'Esperando a tu oponente' : 'Tu oponente es: ' + contrincante}</p>
      )}
      {loading && (
